...
int get_server_addr()
{
    ...
    if (0 != getaddrinfo("192.168.0.2", "5555", &hints, &servinfo) || servinfo == NULL)
    	return 0;
...
int tx(const uip_ipaddr_t *dst_addr, uint16_t port, void *buf, int len)
{
    /* assert (g_sock != -1); */
    ...
    */
    ...
    ret = sendto(g_sock, buf, len, 0, (struct sockaddr *) &g_server_addr, g_server_sl/*sizeof(g_addr)*/);
...
int main()
{
	...
	uint16_t my_port = 1000, dst_port=5683;
	...
	coap_set_header_uri_path(message, "/app_config_info");
