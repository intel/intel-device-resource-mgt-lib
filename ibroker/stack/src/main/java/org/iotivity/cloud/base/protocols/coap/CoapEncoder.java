...
public class CoapEncoder extends MessageToByteEncoder<CoapMessage> {
  ...
  @Override
  protected void encode(ChannelHandlerContext ctx, CoapMessage msg, ByteBuf out) throws Exception {
  ...
  private void calcShimHeader(CoapMessage coapMessage, ByteBuf byteBuf, long length) {
    if (length < 13) {
      byteBuf.writeByte(((int) length & 0x0F) << 4 | (coapMessage.getTokenLength() & 0x0F));
    } else if (length < 269) {
      byteBuf.writeShort(
          (13 & 0x0F) << 12
      ...
      byteBuf.writeByte((14 & 0x0F) << 4 | (coapMessage.getTokenLength() & 0x0F));
      byteBuf.writeShort(((int) length - 269) & 0xFFFF);
    } else if (length < 4294967294L) {
      byteBuf.writeByte((15 & 0x0F) << 4 | (coapMessage.getTokenLength() & 0x0F));
      ...
      throw new IllegalArgumentException("Length must be less than 4GB " + length);
    }
  }

  private void encodeOptions(ByteBuf byteBuf, CoapMessage coapMessage) throws Exception {
    int preOptionNum = 0;
      ...
      if (values != null) {
        if (values.size() > 0) {
          for (byte[] value : values) {
            writeOption(i - preOptionNum, value != null ? value.length : 0, byteBuf, value);
            preOptionNum = i;
          }
  ...
  private void writeOption(int optionDelta, int optionLength, ByteBuf byteBuf, byte[] value) {

    if (optionDelta < 13) {
      if (optionLength < 13) {
        byteBuf.writeByte(((optionDelta & 0xFF) << 4) | (optionLength & 0xFF));
      ...
      throw new IllegalArgumentException("Unsupported option delta " + optionDelta);
    }
