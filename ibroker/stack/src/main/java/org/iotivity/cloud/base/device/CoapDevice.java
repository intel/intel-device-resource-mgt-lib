...
public class CoapDevice extends Device {
  ...
  public void removeObserveChannel(IRequestChannel channel) throws ClientException {
    if (channel != null && mObserveChannelList.contains(channel)) {
      ...
      while (iterator.hasNext()) {
        ...
        if (coapClient.isObserveRequest(token) != null) {
          ...
          coapClient.onResponseReceived(
              MessageBuilder.createResponse(coapRequest, ResponseStatus.CONTENT, null, null));
        }
      }
  ...
  public void sendResponse(IResponse response) {
    ...
    while (iterator.hasNext()) {
      ...
      if (respToken.equals(token) && coapResponse.getSequenceNumber() == -1) {
        iterator.remove();
      }
    }
  ...
  public void onDisconnected() {
    for (IRequestChannel serverChannel : mObserveChannelList) {
      ...
      while (iterator.hasNext()) {
        ...
        if (coapClient.isObserveRequest(token) != null) {
          CoapRequest coapRequest = (CoapRequest) mObserveRequestList.get(token);
          coapRequest.setObserve(Observe.UNSUBSCRIBE);
          coapRequest.setToken(Bytes.longTo8Bytes(token));
          serverChannel.sendRequest(
              MessageBuilder.modifyRequest(coapRequest, null, null, null, null), this);
        }
      }
    }
  }
