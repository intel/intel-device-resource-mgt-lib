...
/** This class provides a set of APIs to print out logs for CoAP request and response. */
@Sharable
public class CoapLogHandler extends ChannelDuplexHandler {

  static final int MAX_LOGLEN = 128;
      ...
      strBuilder.append(
          " FRMT: "
              ...
              + new String(
                  request.getPayload(),
                  0,
                  request.getPayloadSize() > MAX_LOGLEN ? MAX_LOGLEN : request.getPayloadSize()));
    }

    strBuilder.append(" OPT: " + request.getOptionsString());

    return strBuilder.toString();
  }

  private String composeCoapResponse(String channelId, CoapResponse response) {
    StringBuilder strBuilder = new StringBuilder();
      ...
      strBuilder.append(
          " SZ:"
              ...
              + new String(
                  response.getPayload(),
                  0,
                  response.getPayloadSize() > MAX_LOGLEN ? MAX_LOGLEN : response.getPayloadSize()));
    }

    strBuilder.append(" OPT: " + response.getOptionsString());
