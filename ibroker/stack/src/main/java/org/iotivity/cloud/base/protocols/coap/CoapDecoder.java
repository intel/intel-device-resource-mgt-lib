...
public class CoapDecoder extends ByteToMessageDecoder {
  ...
  private int parseOptions(CoapMessage coapMessage, ByteBuf byteBuf, int maxLength) {
    ...
    while (firstByte != 0xFF && maxLength > 0) {
      ...
      if (optionDelta == 13) {
        ...
        optionDelta = 269 + ((byteBuf.readByte() & 0xFF) << 8) + (byteBuf.readByte() & 0xFF);
      }

      if (optionLength == 13) {
        optionLength = 13 + byteBuf.readByte() & 0xFF;
      } else if (optionLength == 14) {
        optionLength = 269 + ((byteBuf.readByte() & 0xFF) << 8) + (byteBuf.readByte() & 0xFF);
      }
