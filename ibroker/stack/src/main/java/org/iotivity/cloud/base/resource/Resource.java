...
public class Resource implements IRequestEventHandler {
  ...
  public interface Functional {
    void queryHandler(Device srcDevice, IRequest request) throws ServerException;
  }

  private HashMap<List<String>, Functional> mQuerySetList = new HashMap<>();

  public void addQueryHandler(List<String> querySet, Functional queryHandler) {
  ...
  @Override
  public final void onRequestReceived(Device srcDevice, IRequest request) throws ServerException {
    ...
    if (reqQuerySet != null && mQuerySetList.size() > 0) {
      for (Entry<List<String>, Functional> entry : mQuerySetList.entrySet()) {
  ...
  public void onDefaultRequestReceived(Device srcDevice, IRequest request) throws ServerException {
    // No default handlers registered, send error
    throw new NotFoundException("No handlers registered");
  }

  public boolean checkQueryException(String property, HashMap<String, List<String>> queryData) {
    return checkQueryException(Arrays.asList(property), queryData);
  }

  public boolean checkQueryException(
      List<String> propertyList, HashMap<String, List<String>> queryData) {

    if (queryData == null) throw new BadRequestException("queryData is null");

    for (String property : propertyList) {
      if (!queryData.containsKey(property))
        throw new PreconditionFailedException(property + " property is not include");
      if (queryData.get(property) == null)
        throw new PreconditionFailedException(property + " param is null");
    }
    return true;
  }

  public boolean checkPayloadException(String property, HashMap<String, Object> payloadData) {
    return checkPayloadException(Arrays.asList(property), payloadData);
  }

  public boolean checkPayloadException(
      List<String> propertyList, HashMap<String, Object> payloadData) {

    if (payloadData == null) throw new BadRequestException("payloadData is null");
